...
export function buildCycleReview(opts: {
  ...
  const housingAmt = livingMap.get("housing") ?? 0;
  const housingPct = pct(housingAmt, buckets.income);
  const dailyLiving = buckets.expense / days;
  const runwayDays = dailyLiving > 0 ? Math.round(Math.max(0, left) / dailyLiving) : 0;
  ...
  if (incomeDelta !== 0 && livingDelta > 0 && Math.abs(livingDelta) > Math.abs(incomeDelta)) {
    patterns.push("Lifestyle creep: living rose faster than income versus last cycle.");
  }
  ...
  if (housingAmt && buckets.income) patterns.push(`Housing is ${housingPct}% of income this cycle (common yardstick is ~30%).`);
  ...
  else if (buckets.income) suggestions.push(`Wants are ${wantPct}% of income — inside the 30% band.`);
  ...
  const over = budgetRows.filter((r) => r.budget > 0 && r.spent > r.budget);
  ...
  if (housingPct > 30 && housingAmt) {
    suggestions.push("Housing is above the 30% rule of thumb. Treat rent/mortgage as the first envelope, not the leftover.");
  }
  if (!budgetRows.length) suggestions.push("Set two or three category envelopes next cycle so variance has a target, not just a leftover.");
  ...
  if (buckets.income && dailyLiving > buckets.income / days) watchouts.push("Daily living is outrunning daily income in this window.");
  ...
  if (pct(debt, buckets.income) >= 20) {
    watchouts.push("Debt service is 20%+ of income. Extra principal beats new envelopes until that share drops.");
  }
  if (!watchouts.length) watchouts.push("No red flags in this window. Keep the same pay-cycle habit and review again next cycle.");

  const extras: string[] = [
    `50/30/20 board this cycle: needs ${needPct}% / wants ${wantPct}% / save+invest ${savePct}%.`,
    left >= 0
      ? `Leftover covers about ${runwayDays} day${runwayDays === 1 ? "" : "s"} of living at this cycle’s daily rate (${money(dailyLiving, currency)}/day).`
      : "No leftover buffer — next payday should fund savings before living.",
    budgetRows.length
      ? `${budgetRows.filter((r) => r.delta < 0).length} envelope${budgetRows.filter((r) => r.delta < 0).length === 1 ? "" : "s"} over, ${budgetRows.filter((r) => r.delta >= 0).length} under.`
      : "No envelopes set — variance is a leftover, not a plan.",
  ];
