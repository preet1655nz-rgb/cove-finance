...
function normalizeNeedle(raw: string) {
  return raw
    .trim()
    .toLowerCase()
    .replace(/[’']/g, "'")
    .replace(/\s+/g, " ")
    .replace(/\bair\s*b\s*n\s*b\b/g, "airbnb")
    .replace(/\bair\s*bnb\b/g, "airbnb");
}
...
function matching(txs: Transaction[], needle: string) {
  ...
  return txs.filter((t) => {
    ...
    const payee = prettyPayee(t.note).toLowerCase();
    const hay = `${note} ${party} ${payee}`;
    ...
    return (
      ...
      payee.includes(n) ||
      hay.includes(n) ||
      (compact.length >= 3 && hayCompact.includes(compact))
    );
  });
}

function handleFind(input: string, q: string, ctx: BrainContext): BrainEffect | null {
  const findLike =
    /\b(find|search|look\s*for|show|list|locate|is there|any|can (you |i )?find|where is|where are|do i have)\b/.test(q) &&
    /\b(transaction|transactions|entry|entries|payment|payments|spend|spent|payee|receipt|charge|charges)\b/.test(q);
  const findFor =
    /\b(find|search|look\s*for|show|list)\b.{0,40}\b(for|of|from|to|at)\b/.test(q) ||
    /\b(transaction|entry|payment|charge).{0,20}\b(for|from|to|at)\b/.test(q) ||
    /\b(can find|find transaction|find entry)\b/.test(q);

  if (!findLike && !findFor) {
    if (!(/\?/.test(input) && /\b(find|search|any|is there|do i have)\b/.test(q))) return null;
  }

  let needle = "";
  const patterns = [
    /(?:find|search|look\s*for|show|list|locate)\s+(?:(?:the|any|my|a|an)\s+)?(?:transaction|transactions|entry|entries|payment|payments|spend|charge)?s?\s*(?:for|of|from|to|at|named|called)?\s*(.+?)\??$/i,
    /(?:transaction|entry|payment|charge)s?\s+(?:for|from|to|at|named|called)\s+(.+?)\??$/i,
    /(?:is there|any|do i have|can (?:you |i )?find)\s+(?:(?:a|an|any|the)\s+)?(?:transaction|entry|payment)?s?\s*(?:for|from|to|at)?\s*(.+?)\??$/i,
  ...
  for (const re of patterns) {
    ...
    if (m?.[1]) {
      needle = m[1]
        .replace(/^(the|a|an|my|any)\s+/i, "")
        .replace(/\b(transaction|transactions|entry|entries|payment|payments)\b/gi, " ")
        .replace(/[?.!]+$/, "")
        .trim();
  ...
  if (needle.length < 2) {
    needle = q
      .replace(/\b(can|you|i|find|search|look|for|show|list|transaction|transactions|entry|entries|payment|payments|is|there|any|do|have|the|a|an|my)\b/g, " ")
      .replace(/[?.!]+/g, " ")
      .replace(/\s+/g, " ")
      .trim();
  }
  ...
  if (!hits.length) {
    return {
      reply: `No transactions matching “${prettyPayee(needle)}”. Try a shorter word from the payee name, or check Activity.`,
      handled: true,
    };
  }
  const sorted = [...hits].sort((a, b) => (a.date < b.date ? 1 : -1));
  const out = sum(sorted.filter((t) => t.type === "expense"));
  const inn = sum(sorted.filter((t) => t.type === "income"));
  const lines = sorted.slice(0, 12).map((t) => `• ${describeTx(t, ctx.currency)}`).join("\n");
  const more = sorted.length > 12 ? `\n…and ${sorted.length - 12} more.` : "";
  const bits: string[] = [];
  if (out) bits.push(`${fmt(out, ctx.currency)} out`);
  if (inn) bits.push(`${fmt(inn, ctx.currency)} in`);
  return {
    reply: `Found ${sorted.length} matching “${prettyPayee(needle)}”${bits.length ? ` · ${bits.join(", ")}` : ""}:\n${lines}${more}`,
    handled: true,
  };
}
...
export function interpretChat(input: string, ctx: BrainContext): BrainEffect {
  ...
  if (!q || /^(help|what can you do|\?)$/i.test(q)) {
    return {
      reply:
        "I can add, edit or delete entries, find transactions, add categories, and analyse patterns. Try “find airbnb”, “add uber income $400”, or “how am I doing?”.",
      handled: true,
    };
  }

  if (/^(hi|hello|hey)\b/.test(q)) {
    return { reply: "Cove here — I can see your ledger. Add, edit, or delete entries, find payees, or ask what the numbers mean.", handled: true };
  }

  if (ctx.pending) {
    const isNewCommand = /^(add|log|record|enter|create|edit|change|update|delete|remove|how|what|set|tax|gst|list|remember|find|search)\b/.test(q);
    if (!isNewCommand) {
  ...
  const found = handleFind(text, q, ctx);
  ...
  if (snap.entryCount) {
    const leftover = q
      .replace(/\b(can|you|i|please|find|search|show|list|the|a|an|my|is|there|any|do|have|transaction|transactions|entry|entries|payment|for|from|to|at|about|of)\b/g, " ")
      .replace(/[?.!]+/g, " ")
      .replace(/\s+/g, " ")
      .trim();
    if (leftover.length >= 3) {
      const hits = matching(ctx.transactions, leftover);
      if (hits.length) {
        const sorted = [...hits].sort((a, b) => (a.date < b.date ? 1 : -1));
        const lines = sorted.slice(0, 10).map((t) => `• ${describeTx(t, ctx.currency)}`).join("\n");
        return {
          reply: `Found ${sorted.length} matching “${prettyPayee(leftover)}”:\n${lines}`,
          handled: true,
        };
      }
      return {
        reply: `Nothing matching “${prettyPayee(leftover)}” in the ledger. Try Activity, or ask “how am I doing?” for a full summary.`,
        handled: true,
      };
    }
